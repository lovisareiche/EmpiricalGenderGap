...
%
%   Copyright 2013-2021 Inmaculada C. �?lvarez, Javier Barbero, José L. Zofío
...
%
    ...
    switch(est.method)
        ...
        case 'TIP'
            fprintf('Panel: Fixed effects with exogenous time invariant variables (TIP)'); 
        case 'HMT'
            fprintf('Panel: Hausman Taylor (1981) model')
        % IV Panel
        case 'PO2SLS'
    ...
    if est.isPanel && (strcmpi(est.method,'RE') || strcmp(est.method,'RE2SLS') || strcmp(est.method,'EC2SLS'))
        if ~est.isSpatial
    ...
    print.names = est.xnames';
    print.coef = est.coef;
    print.stderr = est.stderr;
    print.stat = test.value;
    print.p = test.p;
