...
# Manually added!!!!
coefficients <- c(i$coefficients["shop_groceries"],i$coefficients["fin_lit:shop_groceries"],i$coefficients["shop_major"],i$coefficients["fin_lit:shop_major"],i$coefficients["prep_meals"],i$coefficients["fin_lit:prep_meals"],i$coefficients["decide_finance"],i$coefficients["fin_lit:decide_finance"])  # Coefficients
std_errors <- c(0.5,0.75,0.64,0.96,0.46,0.69,0.55,0.82)  # Standard errors
coef_names <- c("shop_groceries","fin_lit:shop_groceries","shop_major","fin_lit:shop_major","prep_meals","fin_lit:prep_meals","decide_finance","fin_lit:decide_finance")  # Coefficient names


# Define the order of coefficients
coef_order <- factor(coef_names, levels = coef_names)
...
df <- data.frame(coefficients = coefficients, ci_lower = ci_lower, ci_upper = ci_upper, coef_names = coef_order)

# Plotting
plot <- ggplot(df, aes(x = coef_names, y = coefficients, ymin = ci_lower, ymax = ci_upper)) +
...
# Save the plot
ggsave(file.path(outline,"plot.png"), plot, width = 20, height = 12, units = "cm")
